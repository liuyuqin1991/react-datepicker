...
import { isFunction as _isFunction, get as _get, includes as _includes, times as _times } from 'lodash';
...
const MonthPanel: React.FC<MonthPanelProps> = (props) => {
    ...
    computeCellArray().map((month: TD, index: number) => {
      ...
      if(_includes(['day', 'week', 'month'], selectionMode)){
        ...
        if(index !== 0 && ( index + 1 ) % 3 === 0){
          ...
          quarter.push(
            <tr key={`quarter-${index / 3}`} className={classnames(cls)} onClick={() => onDatePick(month, selectionMode)}>
              {arrTemp}
            </tr>
          );
          arrTemp = [];
  ...
  return (
    <table
      cellSpacing="0"
      cellPadding="0"
      className={classnames('month-table', {'is-quarter-mode': selectionMode === 'quarter'})}>
      <tbody>
        {renderTbody()}
      </tbody>
    </table>
