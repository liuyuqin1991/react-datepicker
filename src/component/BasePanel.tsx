...
const BasePanel: React.FC<BasePanelProps> = (props) => {
  ...
  const datePick = (d: Dayjs[]) => {
    // 日，周视图点击年视图进入时
    if (
      _includes(['day', 'week', 'daytime'], selectionMode) &&
      currentPanel === 'year'
    ) {
    ...
    // 月，季视图点击年视图进入时
      ...
      _includes(['month', 'quarter'], selectionMode) &&
      currentPanel === 'year'
    ) {
      setDate(date.year(d[1].year()));
      ...
      <>
        <div className="header">
          ...
          <div className="date-label">
            <div
              className="year"
              onClick={(e: React.BaseSyntheticEvent) => showPanel(e, 'year')}
            >
              {`${date.year()} 年 `}
            </div>
            <div
              className="month"
              onClick={(e: React.BaseSyntheticEvent) => showPanel(e, 'month')}
            >
              {`${date.month() + 1} 月 `}
            </div>
          </div>
        ...
        <div className="header">
          ...
          <div className="date-label">
            <div className="year disabled">
              {`${_toInteger(date.year() / 10)}0年 - ${_toInteger(
                date.year() / 10
              )}9年`}
            </div>
          </div>
  ...
  return <div className="picker-panel">{renderPanel()}</div>;
};
